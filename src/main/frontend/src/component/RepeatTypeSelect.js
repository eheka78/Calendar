        ...
        <div>
            ...
            >
                ...
                <option value="daily" key="daily"
                        disabled={isDailyBlock || !dateDiff || dateDiff === 0}>
                매일</option>
                <option value="weekly" key="weekly"
                        disabled={isWeeklyBlock || !dateDiff || dateDiff === 0}>
                매주</option>
                <option value="monthly" key="monthly"
                        disabled={isMonthlyBlock || !dateDiff || dateDiff === 0}>
                매달</option>
                <option value="yearly" key="yearly"
                        disabled={isYearlyBlock || !dateDiff || dateDiff === 0}
                >매년</option>
            </select>
        </div>
    );
